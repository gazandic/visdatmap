...
class ImageProcessor(object):
    ...
    # sharpen image first
    def process_image(self, image, magnitude=1.88):
        # image = image.convert('RGB')
        # image.filter(ImageFilter.SHARPEN)
        # image = self._resize_image(image, 1.35)
        ...
        bw = gray.point(lambda x: 0 if x<10 else 250, '1')
        cleaned_image_name = '_cleaned.jpg'

        bw = self._resize_image(bw, magnitude)
   ...
   # resize image with magnitude
    def _resize_image(self, image, magnitude):
        if (image.size[0] < 200 ):
            magnitude *= 200/image.size[0]
        elif (image.size[1] < 200 ):
            magnitude *= 200/image.size[1]
        xDim = image.size[0] * magnitude
        yDim = image.size[1] * magnitude
        newSize = (xDim,yDim) # self.aspectRatio(xDim, yDim)
